...
# ---------------------------------------------------------------------------
# Parser exceptions during validation must NOT escape dispatch
#
# Regression: prior to this guard, a model output with an unterminated
# frontmatter (opening --- but no closing ---) caused parse_file() to raise
# UnterminatedFrontmatter inside _validate_output. The exception escaped
# all the way to the orchestrator's outermost try/except, marking the run
# "orchestrator crashed: ..." instead of triggering the documented one-retry
# schema-failure path. Same hazard for malformed YAML in the frontmatter.
# ---------------------------------------------------------------------------


_UNTERMINATED_FRONTMATTER = (
    "---\n"
    "alternative_shapes:\n"
    "  - 'foo'\n"
    "\n"
...
def _patch_invoke_with_contents(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    contents: list[str],
) -> list[WorkerInvocation]:
    """Each call writes the next content to the output file, then returns success."""
    captured: list[WorkerInvocation] = []
    it: Iterator[str] = iter(contents)

    def fake_invoke(
        inv: WorkerInvocation, sp: str, tools: list[str]
    ) -> WorkerResult:
        captured.append(inv)
        inv.output_path.write_text(next(it), encoding="utf-8")
        return WorkerResult(
            exit_code=0,
...
def test_unterminated_frontmatter_does_not_crash_dispatch_recovers_on_retry(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    inv = _make_inv(tmp_path)
    calls = _patch_invoke_with_contents(
        monkeypatch, tmp_path, [_UNTERMINATED_FRONTMATTER, _VALID_OUTPUT]
    )
...
def test_unterminated_frontmatter_persists_returns_schema_failure_no_crash(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    inv = _make_inv(tmp_path)
    _patch_invoke_with_contents(
        monkeypatch,
        tmp_path,
        [_UNTERMINATED_FRONTMATTER, _UNTERMINATED_FRONTMATTER],
    )

    result = dispatch_worker(inv, "sys", [], output_schema=_SCHEMA)

    assert result.schema_valid is False
    assert any("frontmatter" in e.lower() for e in result.schema_errors)


def test_malformed_yaml_frontmatter_does_not_crash_dispatch(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    inv = _make_inv(tmp_path)
    calls = _patch_invoke_with_contents(
        monkeypatch, tmp_path, [_MALFORMED_YAML, _VALID_OUTPUT]
    )
...
def test_parser_error_fed_back_to_retry_context(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    inv = _make_inv(tmp_path)
    captured = _patch_invoke_with_contents(
        monkeypatch, tmp_path, [_UNTERMINATED_FRONTMATTER, _VALID_OUTPUT]
    )
    ...
    assert len(captured) == 2
    retry_ctx = captured[1].extra_context
    assert "schema_errors" in retry_ctx
    assert "frontmatter" in retry_ctx["schema_errors"].lower()
